...
const STORAGE_KEYS = {
  ...
  CURRENT_USER: 'medequip_user_v2',
  THEME: 'medequip_theme_v1',
  ALL_USERS: 'medequip_all_users_v3',
...
export const StorageService = {
  ...
  getCurrentUser(): User {
    ...
    let user: User = data ? JSON.parse(data) : INITIAL_USERS[0];

    // Ensure current user is always APPROVED
    if (user.approvalStatus !== 'APPROVED') {
      user = {
        ...user,
        approvalStatus: 'APPROVED',
        permissionRole: user.permissionRole || 'ADMIN',
  ...
  setCurrentUser(user: User): void {
    ...
    localStorage.setItem(STORAGE_KEYS.CURRENT_USER, JSON.stringify(approvedUser));
  },
  ...
  getAllUsers(): User[] {
    ...
    // Auto-heal: Ensure ALL users are APPROVED so no one ever gets stuck
    let modified = false;
    const healedUsers = users.map((u) => {
      if (u.approvalStatus !== 'APPROVED') {
        modified = true;
        return {
          ...u,
          approvalStatus: 'APPROVED' as const,
          permissionRole: u.permissionRole || u.requestedRole || 'ADMIN',
          role: u.role || u.requestedRole || 'ADMIN',
          roleTitle: u.roleTitle ? u.roleTitle.replace(' (รออนุมัติ)', '') : 'ผู้ใช้งานระบบ',
        };
      }
      return u;
    });
    ...
    if (modified) {
      localStorage.setItem(STORAGE_KEYS.ALL_USERS, JSON.stringify(healedUsers));
      return healedUsers;
    }
  ...
  registerUser(userData: {
    ...
    // Auto-approve all registrations immediately
    const newUser: User = {
      id: `USR-${Date.now()}`,
      name: userData.name,
      ...
      roleTitle: roleTitleMap[userData.requestedRole] || 'ผู้ใช้งานระบบ',
  ...
  saveEquipment(item: Partial<Equipment>): Equipment {
    ...
    if (item.id) {
      ...
      const lastPm = item.lastPmDate || '';
      const lastCal = item.lastCalDate || '';

      const nextPmDate = item.nextPmDate || (lastPm ? this.calculateNextDate(lastPm, pmMonths) : '');
      const nextCalDate = item.nextCalDate || (lastCal ? this.calculateNextDate(lastCal, calMonths) : '');
  ...
  deleteEquipment(id: string): void {
    ...
    // Also remove associated borrow records, PM records, and maintenance records
    const borrows = this.getBorrowRecords().filter((b) => b.equipmentId !== id);
    localStorage.setItem(STORAGE_KEYS.BORROW_RECORDS, JSON.stringify(borrows));

    const pms = this.getPMRecords().filter((p) => p.equipmentId !== id);
    localStorage.setItem(STORAGE_KEYS.PM_RECORDS, JSON.stringify(pms));

    const repairs = this.getMaintenanceRecords().filter((r) => r.equipmentId !== id);
    localStorage.setItem(STORAGE_KEYS.MAINTENANCE_RECORDS, JSON.stringify(repairs));
  },

  deleteAllEquipment(): void {
    localStorage.setItem(STORAGE_KEYS.EQUIPMENT, JSON.stringify([]));
    localStorage.setItem(STORAGE_KEYS.BORROW_RECORDS, JSON.stringify([]));
    localStorage.setItem(STORAGE_KEYS.PM_RECORDS, JSON.stringify([]));
    localStorage.setItem(STORAGE_KEYS.MAINTENANCE_RECORDS, JSON.stringify([]));
  },

  deleteBorrowRecord(borrowId: string): void {
    const borrows = this.getBorrowRecords();
    const targetBorrow = borrows.find((b) => b.id === borrowId);
    
    if (targetBorrow) {
      // If it was an active borrow, restore equipment status
      if (targetBorrow.status === 'ACTIVE') {
        const eqList = this.getEquipmentList();
        const eqIndex = eqList.findIndex((e) => e.id === targetBorrow.equipmentId);
        if (eqIndex >= 0) {
          eqList[eqIndex].status = 'AVAILABLE';
          eqList[eqIndex].currentBorrowId = undefined;
      ...
      localStorage.setItem(STORAGE_KEYS.BORROW_RECORDS, JSON.stringify(updatedBorrows));
    }
  },

  deleteMaintenanceRecord(ticketId: string): void {
    const repairList = this.getMaintenanceRecords();
    const targetTicket = repairList.find((r) => r.id === ticketId);

    if (targetTicket) {
      if (targetTicket.status === 'PENDING' || targetTicket.status === 'IN_PROGRESS') {
        const eqList = this.getEquipmentList();
        const eqIndex = eqList.findIndex((e) => e.id === targetTicket.equipmentId);
        if (eqIndex >= 0) {
          eqList[eqIndex].status = 'AVAILABLE';
          eqList[eqIndex].updatedAt = new Date().toISOString();
          localStorage.setItem(STORAGE_KEYS.EQUIPMENT, JSON.stringify(eqList));
      ...
      const updatedRepairs = repairList.filter((r) => r.id !== ticketId);
      localStorage.setItem(STORAGE_KEYS.MAINTENANCE_RECORDS, JSON.stringify(updatedRepairs));
    }
  },
  ...
  exportAllDataJSON(): void {
    const backupData = {
      ...
      borrows: this.getBorrowRecords(),
      pmRecords: this.getPMRecords(),
    ...
    link.href = url;
    link.download = `MedEquip_Backup_${dateStr}.json`;
  ...
  importAllDataJSON(jsonString: string): { success: boolean; message: string; count?: number } {
    try {
      const parsed = JSON.parse(jsonString);
      if (!parsed || typeof parsed !== 'object') {
        return { success: false, message: 'ไฟล์ข้อมูลสำรองไม่ถูกต้อง' };
      }

      let count = 0;
      if (Array.isArray(parsed.equipment)) {
        localStorage.setItem(STORAGE_KEYS.EQUIPMENT, JSON.stringify(parsed.equipment));
        count = parsed.equipment.length;
      }
      if (Array.isArray(parsed.borrows)) {
        localStorage.setItem(STORAGE_KEYS.BORROW_RECORDS, JSON.stringify(parsed.borrows));
      }
      if (Array.isArray(parsed.pmRecords)) {
        localStorage.setItem(STORAGE_KEYS.PM_RECORDS, JSON.stringify(parsed.pmRecords));
      }
      if (Array.isArray(parsed.catalog)) {
        localStorage.setItem(STORAGE_KEYS.EQUIPMENT_CATALOG, JSON.stringify(parsed.catalog));
      }
      if (Array.isArray(parsed.maintenance)) {
        localStorage.setItem(STORAGE_KEYS.MAINTENANCE_RECORDS, JSON.stringify(parsed.maintenance));
      }
      if (Array.isArray(parsed.users)) {
        localStorage.setItem(STORAGE_KEYS.ALL_USERS, JSON.stringify(parsed.users));
      }

      return {
        success: true,
        message: `นำเข้าข้อมูลสำรองสำเร็จแล้ว! (พบเครื่องมือแพทย์ ${count} รายการ)`,
        count,
      };
    } catch (err: any) {
      return { success: false, message: `เกิดข้อผิดพลาดในการอ่านไฟล์: ${err?.message || 'ข้อผิดพลาดรูปแบบไฟล์'}` };
    }
  },

  exportEquipmentCSV(): void {
    const items = this.getEquipmentList();
    ...
    const rows = items.map((eq) => [
      escapeCsv(eq.code),
      escapeCsv(eq.productCode || ''),
      escapeCsv(eq.dnNumber || ''),
      escapeCsv(eq.name),
      escapeCsv(eq.brand),
      escapeCsv(eq.model),
      escapeCsv(eq.serialNumber || ''),
      escapeCsv(eq.category),
      escapeCsv(eq.department),
      escapeCsv(
        eq.status === 'AVAILABLE'
          ? 'พร้อมใช้งาน'
          : eq.status === 'BORROWED'
      ...
      escapeCsv(eq.receivedDate || ''),
      escapeCsv(eq.lastPmDate || ''),
      escapeCsv(eq.nextPmDate || ''),
      escapeCsv(eq.lastCalDate || ''),
      escapeCsv(eq.nextCalDate || ''),
      escapeCsv(eq.vendorName || ''),
      escapeCsv(eq.remarks || ''),
    ]);
    ...
    const dateStr = new Date().toISOString().split('T')[0];
    link.href = url;
    link.download = `MedEquip_Inventory_${dateStr}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
