...
export const StorageService = {
  ...
  getCurrentUser(): User {
    ...
    // Ensure user object has valid fields
    if (!user || !user.email) {
  ...
  getAllUsers(): User[] {
    ...
    // Ensure Khun Butsarakham exists in users list
    const hasAdmin = users.some((u) => u.email.toLowerCase() === 'butsarakham.tc@gmail.com');
    if (!hasAdmin) {
  ...
  registerUser(userData: {
    ...
    const roleTitleMap = {
      ...
      SALES: 'พนักงานขาย / เซลล์ (ลงทะเบียนข้อมูลได้)',
      ADMIN: 'แอดมินโปรแกรม',
    };
    ...
    const newUser: User = {
      ...
      approvalStatus: userData.email.toLowerCase() === 'butsarakham.tc@gmail.com' ? 'APPROVED' : 'PENDING_APPROVAL',
