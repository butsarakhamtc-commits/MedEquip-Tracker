      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-3xl p-5 sm:p-6 shadow-xl text-slate-800 dark:text-slate-200 my-8 space-y-5">
        ...
        <div className="flex items-start justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 flex flex-wrap items-center gap-x-2 gap-y-1">
                <span>ยี่ห้อ: <strong className="text-slate-800 dark:text-slate-200">{equipment.brand}</strong></span>
                <span>|</span>
                <span>รุ่น: <strong className="text-slate-800 dark:text-slate-200">{equipment.model}</strong></span>
                <span>|</span>
                <span>S/N: <strong className="text-slate-800 dark:text-slate-200 font-mono">{equipment.serialNumber}</strong></span>
                {equipment.productCode && (
                  <>
                    <span>|</span>
                    <span>รหัสสินค้า: <strong className="text-purple-600 dark:text-purple-400 font-mono font-bold">{equipment.productCode}</strong></span>
                  </>
                )}
                <span>|</span>
                <span>DN: <strong className="text-blue-600 dark:text-blue-400 font-mono font-bold">{equipment.dnNumber || '-'}</strong></span>
              </p>
            </div>
          </div>
        ...
        {activeTab === 'info' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
              ...
              <div>
                <span className="text-slate-400">รหัสสินค้า (Product Code):</span>
                <p className="font-bold text-purple-400 font-mono">{equipment.productCode || '-'}</p>
              </div>

              <div>
                <span className="text-slate-400">รหัส DN (Delivery Note):</span>
                <p className="font-bold text-blue-400 font-mono">{equipment.dnNumber || '-'}</p>
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-4 border-t border-slate-200 dark:border-slate-800">
          <div>
            {onDeleteEquipment && (
              <button
                ...
                className="px-3.5 py-2 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 text-rose-700 dark:text-rose-300 font-bold border border-rose-200 dark:border-rose-800 rounded-xl text-xs flex items-center gap-1.5 transition-all"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>ลบเครื่องมือนี้</span>
              </button>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => onOpenRecordPMModal(equipment)}
              className="px-3.5 py-2 bg-amber-600/20 hover:bg-amber-600/30 text-amber-800 dark:text-amber-300 border border-amber-500/30 rounded-xl text-xs font-semibold flex items-center gap-1"
            >
              <Wrench className="w-3.5 h-3.5" />
        ...
        {onDeleteEquipment && (
          <ConfirmModal
            ...
            title={`ยืนยันการลบ [${equipment.code}]`}
            message={`คุณต้องการลบเครื่องมือแพทย์ [${equipment.code}] ${equipment.name} ออกจากระบบใช่หรือไม่?`}
            onConfirm={() => {
