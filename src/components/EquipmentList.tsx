...
export const EquipmentList: React.FC<EquipmentListProps> = ({
  ...
  const toggleSelectAllFiltered = (filteredList: Equipment[]) => {
  ...
  const handleDeleteSelected = () => {
    ...
    setConfirmModalConfig({
      isOpen: true,
      title: `ยืนยันลบรายการที่เลือก (${selectedIds.length} รายการ)`,
      message: `คุณแน่ใจหรือไม่ว่าต้องการลบเครื่องมือแพทย์จำนวน ${selectedIds.length} รายการที่เลือกออกจากทะเบียน?`,
      onConfirm: () => {
        selectedIds.forEach((id) => onDeleteEquipment(id));
        setSelectedIds([]);
      },
    });
  };
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {onOpenBackupModal && (
            <button
              ...
              className="px-3.5 py-2.5 bg-sky-50 dark:bg-sky-950/40 hover:bg-sky-100 dark:hover:bg-sky-900/60 text-sky-800 dark:text-sky-200 font-bold rounded-xl text-xs sm:text-sm border border-sky-200 dark:border-sky-800 flex items-center justify-center gap-1.5 transition-all shadow-xs"
              title="ส่งออก/นำเข้าไฟล์สำรองข้อมูล (JSON / Excel)"
            >
              <Database className="w-4 h-4 text-sky-600 dark:text-sky-400" />
              <span>📦 สำรอง/ย้ายข้อมูล</span>
            </button>
          )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 p-3 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 rounded-2xl">
        <div className="flex items-center gap-2">
          <button
            onClick={() => toggleSelectAllFiltered(filteredList)}
            className="px-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-xl transition-all shadow-2xs flex items-center gap-1.5"
          >
            <input
              type="checkbox"
              checked={filteredList.length > 0 && filteredList.every((e) => selectedIds.includes(e.id))}
              onChange={() => {}}
          ...
          {selectedIds.length > 0 && (
            <span className="text-xs font-bold text-teal-700 dark:text-teal-400 bg-teal-100 dark:bg-teal-950 px-2.5 py-1 rounded-lg border border-teal-200 dark:border-teal-800">
        ...
        {selectedIds.length > 0 && !isReadOnly && (
          <button
            ...
            className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs rounded-xl shadow-md transition-all flex items-center gap-1.5 active:scale-95"
          >
          ...
          return (
            <div
              ...
              className={`bg-white dark:bg-slate-900 border ${
                isSelected 
                  ? 'border-teal-500 ring-2 ring-teal-500/30 dark:border-teal-500' 
                  : 'border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'
              } rounded-2xl p-4 shadow-sm space-y-3 transition-all flex flex-col justify-between`}
            >
