        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="relative">
              ...
              {isUserDropdownOpen && (
                <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 border-2 border-blue-300 dark:border-blue-700 rounded-2xl shadow-2xl p-2 z-50">
                  <div className="px-3 py-2 border-b border-slate-200 dark:border-slate-700 mb-1 flex items-center justify-between">
                    <p className="text-xs font-bold text-slate-800 dark:text-slate-200">เข้าสู่ระบบ / สลับผู้ใช้</p>
                    <button
                      onClick={() => {
                        onOpenAuthModal();
                        setIsUserDropdownOpen(false);
                      }}
                      className="px-2 py-0.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-[11px] font-bold shadow-sm"
                    >
                      + เข้าระบบ / ลงทะเบียน
                    </button>
                  </div>
                  ...
                  <div className="space-y-1">
                    {users.map((u) => (
                      <button
                        ...
                        className={`w-full text-left p-2 rounded-xl flex items-center justify-between transition-all text-xs ${
                          currentUser.id === u.id
                            ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-900 dark:text-blue-200 border border-blue-400 font-semibold'
                            : 'hover:bg-slate-100 dark:hover:bg-slate-700/60'
                        }`}
                      >
                        <div className="flex items-center gap-2 overflow-hidden">
                          <div className="w-7 h-7 rounded-lg bg-blue-600 text-white flex items-center justify-center font-bold overflow-hidden shrink-0">
                            {u.avatar ? (
                          ...
                          <div className="overflow-hidden">
                            <p className="font-semibold truncate text-slate-900 dark:text-white">{u.name}</p>
                            <p className="text-[10px] opacity-70 truncate">{u.roleTitle}</p>
                          </div>
                        </div>
                        ...
                        <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold shrink-0 ${
                          ...
                          {u.approvalStatus === 'APPROVED' ? 'อนุมัติ' : 'รอ'}
                        </span>
                      </button>
                    ))}
                  </div>
