...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (found) {
      ...
      setLoginError('ไม่พบบัญชีผู้ใช้งานนี้ในระบบ หรือรหัสผ่านไม่ถูกต้อง');
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-slate-200">
                <p className="text-[11px] font-bold text-slate-600 mb-2 flex items-center gap-1">
                  ...
                  <span>สลับเข้าใช้งานด้วยบัญชีทดลองตัวอย่าง:</span>
                </p>
                ...
                <div className="space-y-1.5">
                  {users.map((u) => (
                    <button
                      ...
                      className={`w-full text-left p-2.5 rounded-xl border-2 transition-all text-xs flex items-center justify-between ${
                        currentUser.id === u.id
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-7 h-7 rounded-lg bg-blue-600 text-white flex items-center justify-center font-bold text-xs shrink-0 overflow-hidden">
                          {u.avatar ? <img src={u.avatar} alt={u.name} className="w-full h-full object-cover" /> : u.name.charAt(0)}
                        </div>
                        <div className="truncate">
                          <p className="font-bold truncate text-slate-900">{u.name}</p>
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold shrink-0 ${
                        ...
                        {u.approvalStatus === 'APPROVED' ? 'อนุมัติแล้ว' : u.approvalStatus === 'PENDING_APPROVAL' ? 'รออนุมัติ' : 'ถูกปฏิเสธ'}
                      </span>
                    </button>
                  ))}
                </div>
